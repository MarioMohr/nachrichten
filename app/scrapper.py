"""
This script scrapes video feeds for specific channels and synchronizes them.
It resolves handles to channel IDs and maintains a local SQLite database queue.
It also applies a blacklist and cross references duplicate titles.
Relevant videos are inserted with the status new, while irrelevant or duplicate 
videos are inserted as ignored.
"""
...
def init_tables_and_get_columns():
    ...
    Returns a list of existing columns from the processed videos table.
    """
...
def resolve_single_id(handle):
    """
    Takes a channel handle and resolves it to the underlying YouTube channel ID.
    Returns the handle along with the ID or an error message if it fails.
    """
...
def sync_channels_from_file(existing_ids):
    """
    Reads the channels file and synchronizes the active list with the database.
    Adds new channels and removes missing ones from the schema.
    """
...
def fetch_feed(channel_id):
    """
    Fetches the XML feed for a given channel ID.
...
def parse_feed(xml_content):
    """
    Extracts video IDs, titles, and actual published timestamps from the XML entries.
    Returns a list of tuples containing the ID, title, and normalized timestamp.
    """
    ...
    for entry in entries:
        ...
        if v_match and t_match and p_match:
            # Converts ISO 8601 (2026-06-07T12:00:00+00:00) to SQLite standard format (2026-06-07 12:00:00)
...
def run_scraper():
    """
    Main loop that periodically checks the channels for new videos.
    Evaluates blacklist rules and checks for duplicate titles across the DB.
    Inserts newly found videos into the database queue with their actual upload time.
    """
        ...
        for channel_id, handle in channels:
            ...
            for vid_id, title, published_time in entries:
                ...
                if not cursor.fetchone():
                    ...
                    # Cross reference for duplicate titles already in the database
                    ...
                    # Explicitly write the actual YouTube publication timestamp to the discovered_at column
                    cursor.execute(
                    ...
                    # Only increment count if it actually goes into the new queue
                    if ai_status == "new":
                        new_count += 1
